...
class Captcha_solver:
    ...
    def __init__(self,fr_image_url,bg_image_url):
        ...
        add_string = str(time()).replace(".","_")
        self.bg_image_name = f"{add_string}_bg_image.jpg"
        self.fr_image_name = f"{add_string}_fr_image.jpg"
    def load_images(self):
        try:
            bg = r.get(self.bg_image_url)
            fr = r.get(self.fr_image_url)
            if bg.status_code == 200 and fr.status_code == 200:
                with open(f"{self.__current_path}/{self.bg_image_name}","wb") as file:
                    file.write(bg.content)
                    file.close()
                with open(f"{self.__current_path}/{self.fr_image_name}","wb") as file:
    ...
    def prepare_images(self):
        self.bg_img = cv2.imread(f"{self.__current_path}/{self.bg_image_name}")
        self.fr_img = cv2.imread(f"{self.__current_path}/{self.fr_image_name}")

        if os.path.exists(f"{self.__current_path}/{self.bg_image_name}"):
            os.remove(f"{self.__current_path}/{self.bg_image_name}")
        if os.path.exists(f"{self.__current_path}/{self.fr_image_name}"):
            os.remove(f"{self.__current_path}/{self.fr_image_name}")
